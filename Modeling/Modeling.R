...
###### MODEL 1: It only contains area:xxx interaction terms ######
...
# final formula 
final.formula.mod1 <- (Ycount+1) ~ area + std_born_CNT +
  std_high_edu_CNT + std_mid_edu_CNT + std_low_edu_CNT +
  std_cvs_CNT + std_disable_CNT + std_low_CNT + std_midlow_CNT + 
  std_edu_per_kid +
...
# random part
main_scaled_sf$model1.bym.phi <- final.res$summary.random$phi$mean
main_scaled_sf$model1.bym.iid <- final.res$summary.random$iid$mean

# residual part
vec.residual.model.bym <- (main_scaled_sf$Ycount+1) - 
  unlist(final.res$summary.fitted.values$mean, use.names = F)

main_scaled_sf$model1.bym.res <- vec.residual.model.bym
...
###### MODEL 2: the more complex model ######
...
full.formula <- (Ycount+1) ~ area + std_born_CNT + std_divorce_CNT +
  ...
  std_first_inds_CNT + std_second_inds_CNT + std_third_inds_CNT +
  area:std_crime_CNT + area:std_mid_edu_CNT + area:std_low_edu_CNT +
  area:std_disable_CNT + area:std_native_CNT + area:std_edu_per_kid +
  std_divorce_CNT:std_native_CNT + std_low_edu_CNT:std_low_CNT + 
  std_low_CNT:std_native_CNT + std_crime_CNT:std_low_edu_CNT + 
  std_crime_CNT:std_low_CNT + std_crime_CNT:std_disable_CNT +
  std_crime_CNT:std_native_CNT - 1 +
  f(id, model = "bym", graph = g, scale.model = TRUE)

full_tab <- INLA_model_selection(full.formula, main_scaled_sp)
View(full_tab) 

# Kick out std_hosptal_CNT
...
step1.formula <- (Ycount+1) ~ area + std_born_CNT + std_divorce_CNT +
  std_crime_CNT + std_high_edu_CNT + std_mid_edu_CNT + std_low_edu_CNT +
  std_cvs_CNT + std_disable_CNT + std_low_CNT + std_midlow_CNT + std_native_CNT +
  std_plant_rate + std_waterside_rate + std_edu_per_kid +
  std_first_inds_CNT + std_second_inds_CNT + std_third_inds_CNT +
  area:std_crime_CNT + area:std_mid_edu_CNT + area:std_low_edu_CNT +
  area:std_disable_CNT + area:std_native_CNT + area:std_edu_per_kid +
  std_divorce_CNT:std_native_CNT + std_low_edu_CNT:std_low_CNT + 
...
# KICK std_crime_CNT:std_low_CNT
...
step5.formula <- (Ycount+1) ~ area + std_born_CNT + std_divorce_CNT +
  ...
  std_first_inds_CNT + std_second_inds_CNT + std_third_inds_CNT +
  area:std_mid_edu_CNT + area:std_low_edu_CNT +
  area:std_native_CNT + area:std_edu_per_kid +
  std_divorce_CNT:std_native_CNT + std_low_edu_CNT:std_low_CNT + 
  std_low_CNT:std_native_CNT + std_crime_CNT:std_low_edu_CNT +
  std_crime_CNT:std_disable_CNT +
  std_crime_CNT:std_native_CNT - 1 +
...
# std_divorce_CNT:std_native_CNT
...
final.formula.mod2 <- (Ycount+1) ~ area + std_born_CNT +
  std_crime_CNT + std_high_edu_CNT  + std_low_edu_CNT +
  std_cvs_CNT + std_disable_CNT + std_low_CNT + std_midlow_CNT +
  std_edu_per_kid +
  area:std_mid_edu_CNT + area:std_low_edu_CNT +
  std_low_edu_CNT:std_low_CNT + 
  std_low_CNT:std_native_CNT + std_crime_CNT:std_low_edu_CNT +
  std_crime_CNT:std_disable_CNT - 1 +
  f(phi, model = "besag", graph = g, scale.model = T) + 
  f(iid, model = "iid")
...
ggsave("Plot/Model/model2_phi.png", width = 8, height = 6, dpi = 320)
